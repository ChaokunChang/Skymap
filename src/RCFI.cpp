...
void RCFI::load()
{
    QFile Rfile("RFI.csv");
    QFile Cfile("CFI.csv");
    if (!Rfile.open(QIODevice::ReadOnly)||!Cfile.open(QIODevice::ReadOnly))
        init();
    else {
    ...
    for (std::vector<StarPoint>::iterator it = NavStarTable.begin(); it != NavStarTable.end(); it++)
	{
        ...
        if(!neighbour.empty())
        {
            ...
            for (std::vector<StarPoint>::iterator its = neighbour.begin(); its != neighbour.end(); its++)
            {
                ...
                {
                    int tidx=(int)(getSphereAngle(it->x, it->y, its->x, its->y, minPosStar.x, minPosStar.y)/22.5);
                    if (tmpArr[tidx]==0)
                    {
                        tmpArr[tidx] = 1;
                    }
...
int RCFI::find(std::vector<StarPoint> ObserveStarTable,StarPoint target)
{
	...
	{
        canStar.push_back({itm->first,itm->second,256});
	}
	double minAngle = 500;
    StarPoint minPosStar;
    for (std::vector<StarPoint>::iterator its = neighbour.begin(); its != neighbour.end(); its++)
    ...
    for (std::vector<StarPoint>::iterator its = neighbour.begin(); its != neighbour.end(); its++)
	{
        if (its->index != minPosStar.index)
		{
            int tidx=(int)(getSpotAngle(target.x, target.y, its->x, its->y, minPosStar.x, minPosStar.y,focal_length) /22.5);
            if (tmpArr[tidx] == 0)
			{
                tmpArr[tidx] = 1;
			}
...
int RCFI::efind(std::vector<StarPoint> ObserveStarTable,StarPoint target)
{
    ...
    for (std::vector<StarPoint>::iterator its = neighbour.begin(); its != neighbour.end(); its++)
    {
        if (its->index != minPosStar.index)
        {
            int tidx=(int)(getSphereAngle(target.x, target.y, its->x, its->y, minPosStar.x, minPosStar.y) /22.5);
            if (tmpArr[tidx] == 0)
            {
                tmpArr[tidx] = 1;
            }
