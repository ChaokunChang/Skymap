...
bool check_dis(float dis){
    if(dis < SightSize && dis > 1) return true;
    return false;
}

bool is_above_line(float x1,float y1,float x2,float y2,float x3,float y3){
    float k = (y1-y2)/(x1-x2);
    float kt = (y3-y1)/(x3-x1);
    return k>=kt;
}

int star_partition(float x, float y){
...
vector<bool> NoOptic::GetEigenVector(int StarNum){
    ...
    vector<float> AdjacentDistances;
    vector<pair<float,float>> PolarCoordinates;
    float ref_dis=100000;
    int ref_dis_id=0;

    //确定邻星
    for(int i=0;i<SkyStars.size(); i++){
        if(i==StarNum) continue;
        float dis = cal_dis(SkyStars[StarNum].first,SkyStars[StarNum].second,SkyStars[i].first,SkyStars[i].second);
        if(check_dis(dis)){
    ...
    for(int i=0; i<AdjacentStars.size(); i++){
        ...
        PolarCoordinates.push_back(pair<float,float>(r,zeta));
    }
...
int NoOptic::GetMidStar(){
    ...
    for(int i=0; i<ImageStars.size(); i++){
        float dis = cal_dis(x0,y0,ImageStars[i].first,ImageStars[i].second);
...
bool NoOptic::Match(int main_star){
    ...
    for(int i=0;i<ImageStars.size(); i++){
        ...
        float dis = cal_dis(ImageStars[main_star].first,ImageStars[main_star].second,ImageStars[i].first,ImageStars[i].second);
        if(check_dis(dis)){
            //将符合条件的邻星的编号存储到vector中，并记录最近的那颗星。
            if(dis<ref_dis && dis>R0) {
                ref_dis = dis;
